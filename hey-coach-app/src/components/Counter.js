...
export function Interface() {
  const [mealItems, updateMealItems] = useState(
    JSON.parse(localStorage.getItem("meal-list")) || []
  ); //stores all meals as object array
  const [calSum, setCalSum] = useState(
    JSON.parse(localStorage.getItem("cal-sum")) || 0
  ); //stores calorie sum as number
  ...
  function handleAddMeal(name, cal_count) {
    ...
    setCalSum(
      parseInt(calSum) + parseInt(newItems[newItems.length - 1].calories)
    ); // add new calories to old sum and update state
    ...
    return (
      <Box>
        ...
        <ListItem key={id}>
          ...
          <IconButton
            style={{ left: "98%", position: "absolute" }}
            onClick={() => {
  ...
  function updateMeal() {
    ...
    //console.log(oldCals);
    setCalSum(
      parseInt(calSum) + (parseInt(calInput.current.value) - parseInt(oldCals))
    ); //arithmetic to adjust calorie count
    //reset our states
    setIsEditing(false);
  ...
  return (
    //main DOM return
    <Box>
      <h2 className="text-center">Calories In</h2>
      <div className="text-center">{0}cal</div>
      <Paper sx={{ p: 3 }}>
        <h3>Quick Add Meal</h3>
        <Grid container spacing={1}>
          <Grid item xs={6}>
            <Container
              style={{ fontSize: "14px" /*color: mfocused ? '#499c8c' : ''*/ }}
            >
              <TextField
                id="interfaceTF"
          ...
          <Grid item xs={6}>
            <Container
              style={{ fontSize: "14px" /*color: cfocused ? '#499c8c' : ''*/ }}
            >
              <TextField
                id="interfaceTF"
